...
step("Inmunizaciones", async function() {
    await click($('//app-vital-sign/div[@id="@vitalSignLabel.vaccines"]//span[@id="vital-sign-edit"]')) 
});
step("Agregar nuevo <query>", async (query) => {
    await focus(textBox({placeholder:"Nuevo"}))
    await write(query);
});
step("Refuerzos", async function() {
    await click($("//span[@class='icon-plus']"));
    
 });
step("Refuerzo <query>", async (query) => {
    await focus(textBox({placeholder:"Refuerzo"}))
    await write(query);
});
